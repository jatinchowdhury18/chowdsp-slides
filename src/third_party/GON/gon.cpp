...
int32_t GonObject::Int(int32_t _default) const {
    if(type != FieldType::NUMBER) return _default;
    return static_cast<int32_t> (int_data);
}
uint32_t GonObject::UInt(uint32_t _default) const {
    if(type != FieldType::NUMBER) return _default;
    return static_cast<uint32_t> (int_data);
}
